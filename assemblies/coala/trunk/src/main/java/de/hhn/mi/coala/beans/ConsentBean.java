...
public class ConsentBean implements Serializable {
	...
	public String registerConsent() {
		...
		String currentUserName = userService.findCurrent(false).getIdentifier(UserIdentifier.TYPE_USERNAME).getValue();
		final UserIdentifier ui = new UserIdentifier(UserIdentifier.TYPE_USERNAME, currentUserName);
		
		Person p = personService.findByUserIdentifier(ui, false);		
		author = new CoalaAuthor(p.getTitle(), p.getFirstName(), p.getLastName());
